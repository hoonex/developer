...
// Report-only single-variable experiment: keep the exact production PLC and baseline switches,
// adding only TetGen 1.5's explicit mesh-optimization schedule. `-O2/7/3` uses flip level 2,
// all three local operation families (flips, smoothing, insertion/deletion), and at most three
// optimization iterations. The production runner remains unchanged.
const OPTIMIZATION_PROBE_SWITCHES: &str = "-pYzCQO2/7/3";
...
fn quality_snapshot(mesh: &VolumeMesh) -> QualitySnapshot {
    ...
    .expect("optimization probe mesh must yield complete dihedral measurements");
    ...
    .expect("optimization probe mesh must yield complete orthogonality measurements");
    ...
    .expect("optimization probe mesh must yield complete size-transition measurements");
    ...
    .expect("optimization probe mesh must yield complete skewness measurements");
...
fn minimum_dihedral_domain_distance(
    ...
    .expect("optimization probe must retain minimum-dihedral ownership");
    ...
    for axis in 0..3 {
        assert!(
            point[axis].is_finite()
                ...
                && min[axis] <= point[axis]
                && point[axis] <= max[axis],
            "optimization probe hotspot must remain inside the validated domain"
        );
        distance = distance
...
fn temp_root(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "aeroforge-tetgen-optimization-probe-{label}-{}-{nonce}",
        std::process::id()
    ))
}

fn run_optimization_probe(
    executable: &Path,
    label: &str,
    poly_text: &str,
) -> ParsedTetgenVolumeMesh {
    let root = temp_root(label);
    fs::create_dir(&root).expect("optimization probe must allocate a private TetGen directory");
    fs::write(root.join("aeroforge.poly"), poly_text)
        .expect("optimization probe must persist the exact baseline PLC");

    let executable = fs::canonicalize(executable)
        .expect("optimization probe must canonicalize the discovered TetGen executable");
    let output = Command::new(executable)
        .current_dir(&root)
        .arg(OPTIMIZATION_PROBE_SWITCHES)
        .arg("aeroforge.poly")
        .output()
        .expect("optimization probe TetGen process must launch");
    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        output.status.success(),
        "optimization probe TetGen failed: exit={:?}\nstdout:\n{}\nstderr:\n{}",
        output.status.code(), stdout, stderr
    );

    let node = fs::read_to_string(root.join("aeroforge.1.node"))
        .expect("optimization probe must produce .1.node");
    let ele = fs::read_to_string(root.join("aeroforge.1.ele"))
        .expect("optimization probe must produce .1.ele");
    let face = fs::read_to_string(root.join("aeroforge.1.face"))
        .expect("optimization probe must produce .1.face");
    let parsed = parse_tetgen_volume_mesh(&node, &ele, &face)
        .expect("optimization probe output must pass AeroForge parsing/audit");
    fs::remove_dir_all(root).expect("optimization probe private directory must clean up");
    parsed
}
...
fn execute_shape_probe(
    ...
    let baseline = run_project_tetgen_boundary_layer_handoff(state, &executable, policy)
        .expect("baseline boundary-layer handoff must remain valid before optimization probing");
    assert_eq!(baseline.tetgen_run.prepared().switches(), BASELINE_SWITCHES);

    let probe = run_optimization_probe(
        &executable,
        shape,
    ...
    match merge_tetgen_with_boundary_layers(&probe, &baseline.layers, baseline.merge_policy) {
        Ok(merged) => {
            ...
            match validate_candidate_exterior_mesher_handoff(
                ...
                Ok(handoff) => {
                    println!(
                        "AEROFORGE_TETGEN_OPTIMIZATION_PROBE=REPORT_ONLY shape={} scope=merge_contract merge_status=accepted solver_handoff_status=accepted",
                        shape
                    );
                    report_delta(
                        shape,
                ...
                Err(error) => println!(
                    "AEROFORGE_TETGEN_OPTIMIZATION_PROBE=REPORT_ONLY shape={} scope=merge_contract merge_status=accepted solver_handoff_status=rejected error={:?}",
                    shape, error
                ),
            }
        }
        Err(error) => println!(
            "AEROFORGE_TETGEN_OPTIMIZATION_PROBE=REPORT_ONLY shape={} scope=merge_contract merge_status=rejected solver_handoff_status=not_attempted error={:?}",
            shape, error
        ),
    }
}
...
#[test]
fn configured_real_tetgen_builds_desktop_boundary_layer_handoff_optimization_probe_rounded_sphere() {
...
#[test]
fn configured_real_tetgen_builds_desktop_boundary_layer_handoff_optimization_probe_sharp_rim_cylinder() {
    if !real_tetgen_enabled() {
        return;
    }
