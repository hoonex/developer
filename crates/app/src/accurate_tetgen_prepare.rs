...
pub(crate) fn prepare_tetgen_from_state(
    ...
    let generic_handoff = &handoff
        ...
        .handoff
        .handoff;
    let scene_ids = generic_handoff.exterior.scene_object_ids.clone();
    let points = generic_handoff.mesh.points.len();
    let tetrahedra = generic_handoff.mesh.cells.len();
    let boundary_triangles = generic_handoff.mesh.boundary.len();
...
mod tests {
    ...
    fn configured_real_tetgen_prepares_and_persists_desktop_case() {
        ...
        assert!(tetgen.contains("tetra_dihedral_policy_minimum_angle_radians\t0.000000000001"));
        assert!(tetgen.contains("tetra_dihedral_policy_maximum_angle_radians\t3.141592653589793"));
        assert!(tetgen.contains("tetra_dihedral_cells\t"));
        assert!(tetgen.contains("tetra_dihedral_angle_tests\t"));
        assert!(tetgen.contains("tetra_dihedral_observed_minimum_angle_radians\t"));
        assert!(tetgen.contains("tetra_dihedral_observed_minimum_cell\t"));
        assert!(tetgen.contains("tetra_dihedral_observed_minimum_local_edge_start\t"));
        assert!(tetgen.contains("tetra_dihedral_observed_minimum_local_edge_end\t"));
        assert!(tetgen.contains("tetra_dihedral_observed_maximum_angle_radians\t"));
        ...
        assert!(tetgen.contains("tetra_size_transition_policy_maximum_adjacent_cell_volume_ratio\t1000000000000"));
        assert!(tetgen.contains("tetra_size_transition_policy_max_interior_face_tests\t20000000"));
        assert!(tetgen.contains("tetra_size_transition_cells\t"));
        assert!(tetgen.contains("tetra_size_transition_interior_faces\t"));
        assert!(tetgen.contains("tetra_size_transition_interior_face_tests\t"));
        ...
        assert!(tetgen.contains("source_normal_variation_minimum_angle_radians\t0.000001"));
        assert!(tetgen.contains("source_normal_variation_sharp_cutoff_radians\t0.5"));
        assert!(tetgen.contains("source_normal_variation_distance_tolerance\t0.000000001"));
        assert!(tetgen.contains("source_normal_variation_minimum_direction_alignment_cosine\t0.999999"));
        assert!(tetgen.contains("source_normal_variation_maximum_dihedral_angle_difference_radians\t0.000000001"));
        assert!(tetgen.contains("source_normal_variation_max_edge_pair_tests_per_pass\t20000000"));
        assert!(tetgen.contains("source_normal_variation_variation_edge_pair_tests\t"));
        assert!(tetgen.contains("source_normal_variation_sharp_edge_pair_tests\t"));
        assert!(tetgen.contains("source_normal_variation_total_edge_pair_tests\t"));
        assert!(tetgen.contains("source_normal_variation_body_count\t1"));
        assert!(tetgen.contains("source_normal_variation_body_0_scene_object_id\t1"));
        assert!(tetgen.contains("source_normal_variation_body_0_source_variation_edge_count\t"));
        assert!(tetgen.contains("source_normal_variation_body_0_boundary_variation_edge_count\t"));
