...
pub fn draw_accurate_workspace_selector(
    ...
    synchronize_lifecycle(&mut execution, &prepared, &mut workspace);
    ...
    let cancelled_case = workspace
        .last_cancelled_case
        .clone()
        .or_else(|| execution.last_run.as_ref().map(|run| run.case_directory.clone()));

    let ctx = contexts.ctx_mut()?;
    egui::TopBottomPanel::top("accurate_workspace_selector").show(ctx, |ui| {
        ui.horizontal(|ui| {
            ...
            ui.separator();
            draw_execution_status(ui, execution.status);
        });

        if active {
            ui.horizontal(|ui| {
                if let Some(case) = &workspace.active_case {
                    let case_name = case
                        .file_name()
                        .and_then(|name| name.to_str())
                        .unwrap_or("registered case");
                    ui.monospace(case_name).on_hover_text(case.display().to_string());
                } else {
                    ui.weak("Waiting for registered SU2 case…");
                }

                ui.separator();
                if let Some(quality) = &workspace.live_quality {
                    ...
                    let residual = quality
                        ...
                        .unwrap_or_else(|| "n/a".into());
                    ui.monospace(format!(
                        "iter {iteration}/{} · worst RMS {residual} · target {:.4}",
                        quality.requested_iterations, quality.residual_target_log10
                    ));
                } else {
                    ui.weak("Live history not available yet");
                }

                ui.separator();
                match execution.status {
                    AccurateExecutionStatus::Running => {
                        if ui.button("Cancel SU2").clicked() {
                            execution.status = AccurateExecutionStatus::Cancelling;
                        }
                    }
                    AccurateExecutionStatus::Cancelling => {
                        if workspace.cancellation_sent {
                            ui.colored_label(egui::Color32::YELLOW, "Cancellation sent");
                        } else {
                            ui.colored_label(egui::Color32::YELLOW, "Cancellation queued");
                        }
                    }
            ...
            if let Some(error) = &workspace.live_error {
                ui.colored_label(
                    egui::Color32::YELLOW,
            ...
            ui.small(
                "Cancellation targets only the direct SU2_CFD child registered for this case; no process-tree, MPI-worker, pause/resume, or crash-recovery semantics are implied.",
            );
        } else if cancelled {
            ui.horizontal(|ui| {
                ...
                if let Some(case) = &cancelled_case {
                    ...
                    let case_name = case
                        .file_name()
                        .and_then(|name| name.to_str())
                        .unwrap_or("persisted case");
                    ui.monospace(case_name).on_hover_text(case.display().to_string());
                    ui.separator();
                    ui.monospace(LIFECYCLE_PROVENANCE_FILENAME).on_hover_text(
                        case.join(LIFECYCLE_PROVENANCE_FILENAME)
                            .display()
                            .to_string(),
                    );
                }
                if ui.button("Dismiss").clicked() {
                    ...
                    workspace.last_cancelled_case = None;
                    workspace.provenance_error = None;
                }
            });
            if let Some(error) = &workspace.provenance_error {
                ui.colored_label(
                    egui::Color32::YELLOW,
...
fn draw_execution_status(ui: &mut egui::Ui, status: AccurateExecutionStatus) {
    match status {
        AccurateExecutionStatus::Idle => {
...
fn begin_active_run(
    workspace: &mut AccurateWorkspaceUi,
    key: (u64, u64),
    case_root: &str,
) {
    workspace.active_key = Some(key);
    workspace.active_root = Some(PathBuf::from(case_root.trim()));
    workspace.active_case = None;
    workspace.cancellation_sent = false;
    workspace.live_quality = None;
    workspace.live_error = None;
    workspace.last_cancelled_case = None;
    workspace.provenance_error = None;
}
...
fn synchronize_lifecycle(
    ...
    if active {
        ...
        if workspace.active_key != Some(key) {
            begin_active_run(workspace, key, &execution.case_root);
        }

        if workspace.active_case.is_none() {
            if let Some(root) = workspace.active_root.as_deref() {
                match find_registered_active_case(root, revision, sequence) {
                    Ok(Some(case)) => {
                        workspace.active_case = Some(case);
                        workspace.live_error = None;
                    }
                    Ok(None) => {}
                    Err(error) => workspace.live_error = Some(error),
                }
            }
        }

        if let (Some(case), Some(settings)) = (
            workspace.active_case.as_ref(),
            prepared.prepared_settings.as_ref(),
        ) {
            match read_live_quality(case, settings) {
                Ok(Some(quality)) => {
                    workspace.live_quality = Some(quality);
                    workspace.live_error = None;
                }
                Ok(None) => {}
                Err(error) => workspace.live_error = Some(error),
            ...
            if execution.status == AccurateExecutionStatus::Cancelling
                && !workspace.cancellation_sent
            {
                workspace.cancellation_sent = request_su2_case_cancellation(case);
            }
        }
        return;
    }

    let Some((revision, sequence)) = workspace.active_key else {
        return;
    };
    let completed_case = workspace
        .active_case
        .clone()
        .or_else(|| execution.last_run.as_ref().map(|run| run.case_directory.clone()));
    ...
    if let Some(case) = completed_case {
        match take_su2_case_termination(&case) {
            Some(Su2RunTermination::Cancelled) => {
                workspace.last_cancelled_case = Some(case.clone());
                workspace.provenance_error = write_cancelled_lifecycle_provenance(
                    &case,
                    revision,
                    sequence,
                    workspace.live_quality.as_ref(),
                )
                .err()
            ...
            None if execution.status == AccurateExecutionStatus::Cancelled => {
                workspace.provenance_error = Some(format!(
        ...
        workspace.provenance_error = Some(
...
mod tests {
    ...
    fn temp_root(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "aeroforge-workspace-lifecycle-{label}-{}-{nonce}",
            std::process::id()
        ))
    }
    ...
    fn active_run_snapshots_case_root_once() {
        let mut workspace = AccurateWorkspaceUi::default();
        let mut root = String::from("first-root");
        begin_active_run(&mut workspace, (42, 7), &root);
        root.clear();
        root.push_str("second-root");
        assert_eq!(workspace.active_key, Some((42, 7)));
        assert_eq!(workspace.active_root, Some(PathBuf::from("first-root")));
    }
