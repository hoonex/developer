...
mod coarse_shell_support {
    ...
    pub(super) fn build_shifted_xmid_shell(interface_marker: BoundaryMarkerId) -> VolumeMesh {
        ...
        shell
            .audit()
            .expect("shifted-x coarse48 shell must remain a valid VolumeMesh");
        assert_eq!(
            shell
                .boundary
                .iter()
        ...
        .filter(|face| face.marker == interface_marker)
    {
        for &vertex in &face.vertices {
            ...
            let key = point_key_if_half_grid(point)
                .expect("shifted coarse48 interface must remain on the half-unit grid");
...
fn run_middle_tetgen(poly: &str) -> ParsedTetgenVolumeMesh {
    ...
    let executable = discover_tetgen().expect("real TetGen must be discoverable for x-mid probe");
    let epoch_nanos = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap_or_default()
        .as_nanos();
    let work_dir = std::env::temp_dir().join(format!(
        "aeroforge-coarse48-xmid-{}-{epoch_nanos}",
        std::process::id()
    ));
    fs::create_dir(&work_dir).expect("x-mid probe must create a private TetGen directory");

    let result = (|| {
        fs::write(work_dir.join("middle.poly"), poly)
            .expect("x-mid probe must write middle.poly");
        let output = Command::new(&executable)
            .current_dir(&work_dir)
            .arg("-pYzCQ")
            .arg("middle.poly")
            .output()
            .expect("x-mid probe must launch TetGen directly");
        assert!(
            output.status.success(),
            "x-mid TetGen failed: exit={:?} stdout={} stderr={}",
            output.status.code(),
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr),
        );
        let node = fs::read_to_string(work_dir.join("middle.1.node"))
            .expect("x-mid TetGen output must contain middle.1.node");
        let ele = fs::read_to_string(work_dir.join("middle.1.ele"))
            .expect("x-mid TetGen output must contain middle.1.ele");
        let face = fs::read_to_string(work_dir.join("middle.1.face"))
            .expect("x-mid TetGen output must contain middle.1.face");
        aeroforge_accurate_backend::parse_tetgen_volume_mesh(&node, &ele, &face)
            .expect("x-mid TetGen output must satisfy AeroForge parsing/audit")
    })();

    fs::remove_dir_all(&work_dir).expect("x-mid TetGen directory cleanup must succeed");
    result
}
...
fn report_extreme(
    ...
    let (centroid_distance, centroid_plane) = coarse48_interface_distance(centroid);
    let (edge_distance, edge_plane) = coarse48_interface_distance(edge_midpoint);

    println!(
        "AEROFORGE_OUTER_BUFFER_COARSE48_XMID_SHIFT=REPORT_ONLY shape={} kind={} engineering_quality_status=not_established x_mid={} value_rad={} cell={} local_edge={:?} global_edge={:?} vertices={:?} centroid={:?} edge_midpoint={:?} centroid_interface_distance={} centroid_nearest_plane={} edge_interface_distance={} edge_nearest_plane={}",
        shape,
        kind,
        SHIFTED_X_MID,
        value,
...
fn run_xmid_shift_probe(
    ...
    .expect("production BL path must build x-mid probe fixture");
    let handoff = match &prepared_case {
        AccuratePreparedCase::BoundaryLayerTetgen { handoff, .. } => handoff,
        _ => panic!("x-mid probe requires retained BL TetGen handoff"),
    };
    assert_eq!(handoff.layers.len(), 1);
    let layer = &handoff.layers[0];

    let shell = coarse_shell_support::build_shifted_xmid_shell(APP_COARSE_INTERFACE_MARKER);
    ...
    .expect("shifted-x shell must expose dihedral evidence");
    ...
    .expect("shifted-x middle TetGen fill must expose dihedral evidence");

    println!(
        "AEROFORGE_OUTER_BUFFER_COARSE48_XMID_SHIFT_SUMMARY=REPORT_ONLY shape={} engineering_quality_status=not_established x_mid={} shell_cells={} middle_cells={} shell_min_dihedral_rad={} shell_max_dihedral_rad={} middle_min_dihedral_rad={} middle_max_dihedral_rad={}",
        shape,
        SHIFTED_X_MID,
        shell.cells.len(),
...
#[test]
fn configured_real_tetgen_builds_desktop_boundary_layer_handoff_coarse48_xmid_shift_for_rounded_sphere() {
...
#[test]
fn configured_real_tetgen_builds_desktop_boundary_layer_handoff_coarse48_xmid_shift_for_sharp_rim_cylinder() {
    if !env_enabled("AEROFORGE_REQUIRE_REAL_TETGEN") || discover_tetgen().is_none() {
        return;
    }
