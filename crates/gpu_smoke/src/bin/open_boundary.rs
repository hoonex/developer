...
const DIMS: [usize; 3] = [16, 4, 3];
const Q: usize = 19;
const STEPS: usize = 8;
const TAU: f32 = 0.8;
const INLET_VELOCITY: [f32; 3] = [0.03, 0.0, 0.0];
const OUTLET_DENSITY: f32 = 1.0;
...
fn main() {
    ...
    println!(
        "AEROFORGE_GPU_OPEN_ADAPTER name={:?} backend={:?} device_type={:?}",
    ...
    assert!(
        ...
        "AEROFORGE_GPU_OPEN_BOUNDARY_PARITY=NO_COMPUTE adapter={:?}",
        info.name
    );
    ...
    assert!(
        ...
        "AEROFORGE_GPU_OPEN_BOUNDARY_PARITY=INSUFFICIENT_STORAGE_BINDINGS available={} required={}",
    ...
    let (device, queue) = pollster::block_on(adapter.request_device(&wgpu::DeviceDescriptor {
        label: Some("AeroForge GPU NEQ open-boundary parity smoke"),
    ...
    .expect("failed to create wgpu device for AeroForge NEQ parity smoke");
    ...
    let forcing_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("open_forcing"),
        contents: bytemuck::cast_slice(&forcing),
        usage: wgpu::BufferUsages::STORAGE,
    });

    let sample_buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("open_samples"),
        size: sample_bytes,
        usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC,
        mapped_at_creation: false,
    });
    let download_buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("open_download"),
        size: sample_bytes,
        usage: wgpu::BufferUsages::COPY_DST | wgpu::BufferUsages::MAP_READ,
        mapped_at_creation: false,
    });

    let packed_open_masks = VELOCITY_INLET_MASK | (PRESSURE_OUTLET_MASK << PRESSURE_MASK_SHIFT);
    let params = Params {
        dims_stride: [DIMS[0] as u32, DIMS[1] as u32, DIMS[2] as u32, 1],
        control: [cells as u32, 0, 0, packed_open_masks],
        ...
        wall_x_max: [0.0, 0.0, 0.0, OUTLET_DENSITY],
        wall_y_min: [0.0; 4],
        wall_y_max: [0.0; 4],
        wall_z_min: [0.0; 4],
        wall_z_max: [0.0; 4],
    ...
    let params_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("open_params"),
        contents: bytemuck::bytes_of(&params),
        usage: wgpu::BufferUsages::UNIFORM,
    });

    let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("AeroForge LBM NEQ parity layout"),
        entries: &[
    ...
    let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("AeroForge LBM NEQ parity pipeline layout"),
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("AeroForge LBM NEQ parity encoder"),
    });
    let cell_workgroups = (cells as u32).div_ceil(64);
    let face_cells = (DIMS[1] * DIMS[2]) as u32;
    let face_workgroups = face_cells.div_ceil(64);
    let mut ping_is_a = true;
    {
        let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("AeroForge LBM NEQ parity pass"),
    ...
    device
        ...
        .expect("failed while waiting for GPU NEQ parity readback");
    let mapped = slice.get_mapped_range();
    let gpu = decode_samples(&mapped);
    assert_eq!(gpu.len(), cells, "GPU NEQ parity sample count mismatch");

    let mut max_error = 0.0_f32;
    for (i, sample) in gpu.iter().enumerate() {
        assert!(
            sample.iter().all(|value| value.is_finite()),
            "non-finite GPU NEQ sample at cell {i}"
        );
    ...
    assert!(
        ...
        "AEROFORGE_GPU_OPEN_BOUNDARY_PARITY=FAIL max_error={max_error:.8} limit={MAX_ALLOWED_ERROR:.8}"
    );
    println!(
        "AEROFORGE_GPU_OPEN_BOUNDARY_PARITY=PASS inlet_mask={VELOCITY_INLET_MASK} pressure_mask={PRESSURE_OUTLET_MASK} steps={STEPS} cells={cells} max_error={max_error:.8}"
    );
}

fn validate_wgsl(source: &str) {
    let module = naga::front::wgsl::parse_str(source)
        .unwrap_or_else(|error| panic!("AEROFORGE_WGSL_OPEN=PARSE_FAIL {error:?}"));
    naga::valid::Validator::new(
        naga::valid::ValidationFlags::all(),
        naga::valid::Capabilities::all(),
    )
    .validate(&module)
    .unwrap_or_else(|error| panic!("AEROFORGE_WGSL_OPEN=VALIDATION_FAIL {error:?}"));
    println!("AEROFORGE_WGSL_OPEN=PASS");
}
...
fn request_adapter(instance: &wgpu::Instance) -> wgpu::Adapter {
    ...
    match pollster::block_on(instance.request_adapter(&fallback_options)) {
        ...
        Err(fallback_error) => {
            eprintln!(
                "AEROFORGE_GPU_OPEN_FALLBACK_UNAVAILABLE error={fallback_error}; trying default adapter"
            );
            pollster::block_on(instance.request_adapter(&wgpu::RequestAdapterOptions::default()))
                .expect("AEROFORGE_GPU_OPEN_BOUNDARY_PARITY=NO_ADAPTER")
        }
    }
}
...
fn cpu_reference() -> Vec<[f32; 3]> {
    ...
    solver
        ...
        .expect("GPU NEQ parity boundary policy must be valid");
    for _ in 0..STEPS {
        solver.step(&[]);
    }
