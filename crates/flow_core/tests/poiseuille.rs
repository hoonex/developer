...
fn guo_forced_channel_matches_planar_poiseuille_solution() {
    let dims = [16_usize, 8_usize, 3_usize];
    let tau = 0.8_f32;
    ...
    solver
        ...
        .expect("channel boundary policy must be valid");
    ...
    let mut observed = Vec::with_capacity(dims[1]);
    ...
    for y in 0..dims[1] {
        ...
        // Half-way bounce-back places each wall half a lattice cell outside the edge fluid node.
    ...
    for (y, &u) in observed.iter().enumerate() {
        let wall_distance = y as f32 + 0.5;
        let expected = acceleration[0] * wall_distance * (channel_height - wall_distance) / (2.0 * nu);
