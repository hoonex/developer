...
use crate::wall_normal_spacing::{
    validate_body_wall_first_cell_heights, BodyWallFirstCellHeightPolicy,
};
...
#[test]
fn configured_real_tetgen_exposes_body_wall_first_cell_height_evidence() {
    ...
    let Some(executable) = discover_tetgen() else {
        ...
        eprintln!("real TetGen first-cell-height smoke skipped: no executable configured/discovered");
        return;
    };
    ...
    assert!(body.minimum_height >= 1.0e-12);
