...
- optional user-installed external TetGen path with explicit source admission, deterministic PLC/hole seeds, parser/process provenance, volume non-overlap, source/body normal and crease evidence, discrete triangulated normal-variation evidence, first-cell wall-normal height observations, and one-to-one triangulated source-facet ↔ output-body-facet correspondence;
- external TetGen provenance persisted as `aeroforge_tetgen_handoff.tsv` format v8 while mesh fidelity remains explicitly unclassified rather than being silently promoted.
...
The optional external TetGen path now has stronger direct source-surface evidence: routine real-TetGen CI demonstrates one-to-one coincidence between the input triangulated source facets and output body-boundary facets within explicit numerical tolerances, including a rounded 528-triangle fixture. That evidence is stronger than proximity-only correspondence, but it still does **not** establish analytic/CAD surface identity, CAD feature topology, continuous curvature independent of source tessellation, exact edge identity, a layered boundary-layer mesh, y+ suitability, universal engineering mesh-quality thresholds, or engineering CFD accuracy.
